...
variable "lambda_memory_size" {
  ...
  type        = number
  default     = 512  # More memory for prod
}
...
variable "s3_lifecycle_days" {
  ...
  type        = number
  default     = 30  # Keep files longer in prod
}
...
variable "log_retention_days" {
  ...
  type        = number
  default     = 30  # Keep logs for 30 days in prod
}
...
variable "email_check_schedule" {
  ...
  type        = string
  default     = "rate(4 hours)"  # Check emails every 4 hours
}